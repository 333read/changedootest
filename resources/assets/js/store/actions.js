...
export default {
    ...
    getDialogs({state, dispatch, getters}, requestData) {
        return new Promise(function (resolve, reject) {
            // 判断state.userId是否为0，如果为0，则清空state.cacheDialogs，并返回一个错误信息
 if (state.userId === 0) {
                state.cacheDialogs = [];
                reject({msg: 'Parameter error'});
                return;
            }
 // 判断requestData是否为json格式，如果不是，则将其置为空对象
            if (!$A.isJson(requestData)) {
                requestData = {}
            }
 // 判断requestData.page是否为undefined，如果是，则将其置为1
            if (typeof requestData.page === "undefined") {
                requestData.page = 1
            }
 // 判断requestData.pagesize是否为undefined，如果是，则将其置为20
            if (typeof requestData.pagesize === "undefined") {
                requestData.pagesize = 20
            }
 // 调用$callData函数，传入'dialogs'、requestData和state作为参数，并将返回值赋给callData
