<template>
                    ...
                    <div class="dialog-block">
                        <div class="dialog-avatar" @click="onViewAvatar">
                            <template v-if="dialogData.type=='group'">
                                ...
                                <i v-else-if="ƒ.group_type=='task'" class="taskfont icon-avatar task">&#xe6f4;</i>
                        ...
                        <div class="dialog-title">
                            <div class="main-title">
                                ...
                                <Tag v-if="dialogData.bot" class="after" :fade="false">
                                    <!-- 下拉框的条件判断 -->
                                     <div v-if="dialogData.dialog_user.userid == 6">
                                        <el-select v-model="options" class="bot-select" prop="options"  >            
                                            <el-option label="4o高级模型" value="选项1"></el-option>
                                            <el-option label="K-Nearest Neighbors" value="选项2"></el-option>
                                            <el-option label="Mistral" value="选项3"></el-option>
                                            <el-option label="Grok xAI" value="选项4"></el-option>
                                            <el-option label="Kimi" value="选项5"></el-option>
                                        </el-select>
                                     </div>
                                    <div v-else="dialogData.dialog_user.userid != 6" >
                                        {{$L('机器人')}}
                                    </div>
    ...
    <!-- add by hss start 20240911 根据 dialog_user.userid 决定显示消息列表 -->
    <div v-if="dialogData.dialog_user.userid === 6">
        <div class="chatBox" ref="chatBox">
            <div v-for="(message, index) in messages" :key="index" class="message">
                <!-- 机器人消息 -->
                    <div v-if="message.isBot" class="message-content bot-message">
                    <img v-if="message.avatar" :src="message.avatar" alt="Bot Avatar" class="avatar bot-avatar"/>
    ...
    <!-- add by hss end -->
        ...
        <div ref="footer" class="dialog-footer" @click="onActive">
            ...
            <!-- add by hss start 20240911 -->
            <div v-if="dialogData.dialog_user.userid === 6" class="chat-input-container">
...
export default {
    ...
    methods: {
        //by hss 20240911
        async sendMessage() {
        if (!this.userInput.trim()) return;   // 如果用户输入为空，则返回
        ...
        this.messages.push({ 
            ...
            isBot: true,
            avatar:'http://localhost:8090/images/avatar/default_temmol.png',
        ...
        sendMsg(text, type) {
            ...
            let textBody,
                ...
                silence = "no",
                emptied = false;
            // 如果text是字符串且不为空，则将text赋值给textBody
            if (typeof text === "string" && text) {
                textBody = text;
            } else {
                // 否则将this.msgText赋值给textBody，并将emptied设为true
                textBody = this.msgText;
                emptied = true;
            }
            // 如果type为md，则将this.$refs.input.getText()赋值给textBody，并将textType设为md
            if (type === "md") {
                textBody = this.$refs.input.getText()
                textType = "md"
            } else if (type === "silence") {
                // 如果type为silence，则将silence设为yes
                silence = "yes"
            }
            // 如果textBody为空，则调用this.inputFocus()方法，并返回
            if (textBody == '') {
                this.inputFocus();
                return;
            }
            // 如果textType为text，则对textBody进行替换
            if (textType === "text") {
                textBody = textBody
            ...
            //
            // 如果this.dialogData.extra_quote_type为update，则进行修改
            if (this.dialogData.extra_quote_type === 'update') {
                ...
                // 调用this.$store.dispatch("setLoad")方法
                this.$store.dispatch("setLoad", {
                    key: `msg-${update_id}`,
                    delay: 600
                })
                // 调用this.cancelQuote()方法
                this.cancelQuote()
                // 调用this.onActive()方法
                this.onActive()
                //
                // 调用this.$store.dispatch("call")方法
                this.$store.dispatch("call", {
                    ...
                    // 调用this.sendSuccess()方法
                    this.sendSuccess(data, 0, true)
                    // 调用this.onPositionId()方法
